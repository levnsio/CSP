...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        # 计算dice
        dic = 2 * torch.diag(h) / (h.sum(1) + h.sum(0))
        # 计算每个类别的召回率
        rec = torch.diag(h) / h.sum(0)

        return acc_global, acc, rec, dic, iou
    ...
    def __str__(self):
        acc_global, acc, rec, dic, iou = self.compute()
        return (
            'accuracy: {:.4f}\n'
            'precision: {}\n'
            'recall: {}\n'
            ...
            ['{:.4f}'.format(i) for i in (rec * 100).tolist()],
            ['{:.4f}'.format(i) for i in (dic * 100).tolist()],
...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        if self.num_classes == 1:
            ...
            self.cumulative_dice += dice_coeff(pred, dice_target, ignore_index=self.ignore_index)
        else:
